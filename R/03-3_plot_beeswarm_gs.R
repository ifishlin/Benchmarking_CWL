...
## Load data
###############################################$
beta_cov_gs_ds_tb <- read_("beta_cov_gs_tb", "03_GS_preprocessing")
...
plot_gs_comp_metric_condensed <- function(var, prot, title=NA){
  ...
  dist_plot <- beta_cov_gs_ds_tb_ %>% dplyr::filter(var_!=0) %>% dplyr::mutate(cancer=ifelse(sample %in% c("5N", "6N"), "C", "T")) %>%
    dplyr::mutate(sep=paste0(workflow, "_",cancer)) %>%
    ggplot(aes(x = sep, y = var_, color=level, shape=cancer)) +
    ...
    scale_shape_manual(values=c(16, 4)) #+ scale_color_manual(values=c("H" = "red", "M" = "blue", "L" = "green"))
  ...
  save_(
    paste0(var, "_dist_sample_",prot, "_beeswarm"),
    plot=dist_plot,
    use_pdf = TRUE,
    width=7.5,
    height=3
