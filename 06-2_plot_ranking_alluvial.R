...
indv_ranking_tb = indv_ranking_tb_ %>% select(workflow) %>% 
...
b = b %>% mutate(PBAT = factor(paste0(c("BSBolt", "methylCtools", "bwa-meth", "NA", "Bismark", 
                          ...
                          levels = paste0(c("BSBolt", "methylCtools", "FAME", "Biscuit", "Bismark",
                                     "bwa-meth", "methylpy", "GSNAP", "NA"),"   ")))

b = b %>% mutate(Swift = factor(paste0(c("BSBolt/methylCtools", "BSBolt/methylCtools", "bwa-meth", "BAT", "Bismark", 
                       "FAME", "gemBS", "Biscuit", "methylpy", "GSNAP")," "), 
                       levels = paste0(c("bwa-meth", "BSBolt/methylCtools", "BAT", "Bismark", "gemBS", "FAME",
                                  "methylpy", "Biscuit", "GSNAP")," ")))

b$workflow = factor(b$workflow, levels = workflows)

###

g = b %>% ggplot(aes(y = Freq, axis1 = WGBS,
                 axis2 = Swift, axis3 = TWGBS, axis4 = PBAT, axis5 = EMseq)) + 
