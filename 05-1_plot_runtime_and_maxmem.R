...
save_("max_mem", plot=g, use_pdf=TRUE, width=7.5, height=3)
...
g2 = df_run_t %>% 
  ...
  theme(panel.spacing.y = unit(0, "mm"), 
        ...
        breaks = trans_breaks("log2", function(x) 2^x)) +   
  ylab("Run Time (hours)") + 
  geom_hline(yintercept=c(0, 2^4, 2^8), linetype="dashed",size=0.1) 

g2

save_("run_time", plot=g2, use_pdf=TRUE, width=7.5, height=3)
